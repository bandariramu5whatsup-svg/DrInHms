...
namespace HanuMediSoftCore.Services.Hms.Op.Masters
{
    public class ExpensesPaidToService
    {
        ...
        public List<Dictionary<string, object?>> GetExpensesPaidTo(ExpensesPaidTo model)
        {
            ...
            // Using helper class
            DataTable dt = _db.ExecuteSP(ExpensesPaidToProcedures.Get, parameters);
        ...
        public List<Dictionary<string, object?>> SaveExpensesPaidTo(ExpensesPaidTo model)
        {
            ...
            // Using helper class
            DataTable dt = _db.ExecuteSP(ExpensesPaidToProcedures.Save, parameters);
            var list = _db.ToList(dt);
            return list;
        }

        public List<Dictionary<string, object?>> FillExpensesPaidTo(ExpensesPaidTo model)
        {
            var parameters = new Dictionary<string, object?>
                        {
            ...
            // Using helper class
            DataTable dt = _db.ExecuteSP(ExpensesPaidToProcedures.DdGet, parameters);
            var list = _db.ToList(dt);
            return list;
        }
    }
